rm version_issues #remove current version of file, if any. Needed during development, should be able to remove when actually deploying
        ...
        { 
            if ( $0 ~ /\/dev\/null/ ) # is this a module file being uploaded for the first time? 
                ...
                { 
                    ...
                    system( "issue=$(grep '[+-]version:' <<<  \" "extracted_version_lines " \"| tr -d \"[-+]version:\"| bash version_values_comparison.sh); if [ -n \"$issue\" ]; then echo "$1": $issue >> version_issues ; fi") 
