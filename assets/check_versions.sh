...
{ 
    if (system( " ./assets/is_module.sh "$1 ) == 0 ) 
        { 
            if ( $0 ~ /\/dev\/null/ ) # is this a module file being uploaded for the first time? 
                ...
                { 
                    ...
                    system( "issue=$(grep '[+-]version:' <<<  \" "extracted_version_lines " \"| tr -d \"[-+]version:\"| ./assets/version_values_comparison.sh); if [ -n \"$issue\" ]; then echo "$1": $issue >> version_issues ; fi") 
